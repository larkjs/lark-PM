...
class Client {
    ...
    async spawnDaemon() {
        ...
        return new Promise((resolve, reject) => {
            ...
            let timeout = setTimeout(() => reject(new Error('Spawning time out')), this.script.options.timeout);
    ...
    clear() {
        ...
        if (pid) {
            ...
            try {
                ...
                debug(`failing to kill, message: ${e.message}`);
            }
        }
    ...
    async exit() {
        debug('exit');
        let state = await this.checkDaemonState();
