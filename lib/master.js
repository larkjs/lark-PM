...
function main (config, WORKER) {
    ...
    schedulingPolicy = schedulingPolicy.trim().replace(/-/g, '');
    schedulingPolicy = !schedulingPolicy || schedulingPolicy === 'roundrobin' ?
                            cluster.SCHED_RR : cluster.SCHED_NONE;
...
function cli () {
    let socketFile = path.join(path.dirname(process.mainModule.filename), '.' + path.basename(process.mainModule.filename) + '.sock');
    ...
    function stopServer () {
        console.log("Stopping server due to the process's exiting ... ");
        server.close();
    }
